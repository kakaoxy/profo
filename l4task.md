...
- **严重程度**: 🟠 高 - 增加维护成本，容易出错
- **解决方案**:
  1. 后端添加 `field_validator`，支持同时接收字符串或数组格式
  2. 统一转换为逗号分隔的字符串存储
  3. 前端直接使用数组格式，简化转换逻辑
- **状态**: ✅ 已完成
- **验证方式**: 前后端类型一致，表单提交和显示正常
- **修改文件**:
  - `backend/schemas/l4_marketing.py` - 添加 Union 导入和字段验证器
  - `frontend/src/app/(main)/l4-marketing/projects/_components/form-schema.ts` - 更新 schema 和转换函数
...
- **解决方案**: 同 P1-001，使用相同的 field_validator 处理
- **状态**: ✅ 已完成
- **验证方式**: 标签添加、编辑、显示功能正常
- **修改文件**: 同 P1-001
...
- **解决方案**:
  ...
  2. 更新所有路由引用（共修改了 12 个文件中的 19 处路由）
- **状态**: ✅ 已完成
- **验证方式**: 目录重命名后，所有页面和组件正常加载
- **修改文件**:
  - `frontend/src/components/app-sidebar.tsx`
  - `frontend/src/app/(main)/l4-marketing/projects/_components/edit/useMiniProjectForm.ts`
  - `frontend/src/app/(main)/l4-marketing/projects/actions/projects.ts`
  - `frontend/src/app/(main)/l4-marketing/projects/[id]/preview/page.tsx`
  - `frontend/src/app/(main)/l4-marketing/projects/[id]/preview/_components/price-sidebar.tsx`
  - `frontend/src/app/(main)/l4-marketing/projects/_components/action-cell.tsx`
  - `frontend/src/app/(main)/l4-marketing/projects/page.tsx`
  - `frontend/src/app/(main)/l4-marketing/projects/_components/edit/EditMode.tsx`
  - `frontend/src/app/(main)/l4-marketing/projects/[id]/edit/page.tsx`
  - `frontend/src/app/(main)/l4-marketing/projects/new/page.tsx`
  - `frontend/src/app/(main)/l4-marketing/projects/_components/detail/marketing-detail-header.tsx`
  - `frontend/src/app/(main)/l4-marketing/projects/_components/detail/photos-section.tsx`
...
- **严重程度**: 🟡 中 - 影响性能和用户体验
- **解决方案**:
  1. 将 `columns.tsx` 中的 `img` 替换为 Next.js `Image` 组件
  2. 将 `hero-gallery.tsx` 中的 3 处 `img` 替换为 Next.js `Image` 组件
  3. 配置适当的 `sizes` 属性和 `priority` 属性以优化加载
- **状态**: ✅ 已完成
- **验证方式**: 图片正常加载，Lighthouse 性能评分提升
- **修改文件**:
  - `frontend/src/app/(main)/l4-marketing/projects/columns.tsx`
  - `frontend/src/app/(main)/l4-marketing/projects/[id]/preview/_components/hero-gallery.tsx`
...
- **严重程度**: 🟡 中 - 代码风格问题
- **解决方案**: 将枚举类从 `class EnumName(str)` 改为标准的 `class EnumName(str, Enum)` 形式
- **状态**: ✅ 已完成
- **验证方式**: 枚举功能正常，类型检查通过
- **修改文件**:
  - `backend/models/l4_marketing.py` - 添加 `Enum` 导入，修复 `PublishStatus` 和 `MarketingProjectStatus`
  - `backend/schemas/l4_marketing.py` - 添加 `Enum` 导入，修复 `PublishStatus`、`MarketingProjectStatus` 和 `MediaType`
...
- **严重程度**: 🟡 中 - 影响调试和用户体验
- **解决方案**:
  1. 添加 `parseApiError` 函数，支持解析 FastAPI 标准错误格式和 Pydantic 验证错误
  2. 添加 `parseNetworkError` 函数，根据错误类型返回不同的网络错误信息
  3. 所有 action 函数使用新的错误解析函数
- **状态**: ✅ 已完成
- **验证方式**: 不同类型的错误显示不同的提示信息
- **修改文件**:
  - `frontend/src/app/(main)/l4-marketing/projects/actions/projects.ts`
  - `frontend/src/app/(main)/l4-marketing/projects/actions/media.ts`
...
- **严重程度**: 🟢 低 - 轻微冗余
- **解决方案**: 后端 Schema 已包含 `community_name` 字段，移除前端不必要的类型扩展
- **状态**: ✅ 已完成
- **验证方式**: 类型定义简化，功能正常
- **修改文件**: `frontend/src/app/(main)/l4-marketing/projects/types.ts`
...
- **解决方案**:
  1. 定义明确的 `ApiError` 接口
  2. 添加 `isApiError` 类型守卫函数
  3. 添加 `getErrorStatusCode` 辅助函数
  4. 使用类型守卫替代复杂断言
- **状态**: ✅ 已完成
- **验证方式**: 类型检查通过，代码更清晰
- **修改文件**: `frontend/src/app/(main)/l4-marketing/projects/page.tsx`
...
- **解决方案**: React 默认的 JSX 表达式 `{tag}` 已经会自动转义 HTML 特殊字符，XSS 风险已得到控制
- **状态**: ✅ 已完成 (无需额外处理)
- **验证方式**: 安全扫描通过
- **备注**: React 默认已提供保护，无需额外处理
...
| P1-001 | 修复前后端类型不一致 - images 字段 | ✅ 完成 | 后端添加 field_validator 支持数组格式 |
| P1-002 | 修复前后端类型不一致 - tags 字段 | ✅ 完成 | 同 images 字段处理 |
| P1-003 | 修复目录命名不符合规范 | ✅ 完成 | minipro -> l4-marketing，更新 19 处路由引用 |
| P2-002 | 修复图片加载无优化 | ✅ 完成 | img -> Next.js Image 组件 |
| P2-003 | 修复枚举类定义不规范 | ✅ 完成 | class Enum(str) -> class Enum(str, Enum) |
| P2-004 | 修复错误处理不够完善 | ✅ 完成 | 添加 parseApiError 和 parseNetworkError 函数 |
| P3-001 | 服务层行数接近限制 | ✅ 完成 | 作为技术债务记录，暂不处理 |
| P3-002 | 修复类型定义重复 | ✅ 完成 | 移除前端不必要的 community_name 扩展 |
| P3-003 | 修复类型处理不够严谨 | ✅ 完成 | 添加 ApiError 接口和类型守卫函数 |
| P3-004 | XSS 潜在风险 | ✅ 完成 | React 默认已保护，无需额外处理 |
...
- frontend/src/app/(main)/l4-marketing/projects/actions/media.ts
- frontend/src/app/(main)/l4-marketing/projects/[id]/preview/page.tsx
- frontend/src/app/(main)/l4-marketing/projects/_components/form-schema.ts
- frontend/src/app/(main)/l4-marketing/projects/types.ts
- frontend/src/app/(main)/l4-marketing/projects/page.tsx
- frontend/src/components/app-sidebar.tsx
