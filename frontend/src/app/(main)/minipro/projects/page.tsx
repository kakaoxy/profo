...
export default async function ProjectsPage({
  ...
  const items: MiniProject[] = data && 'items' in data ? (data as { items: MiniProject[] }).items || [] : [];
  const total = data && 'total' in data ? (data as { total: number }).total || 0 : 0;
      ...
      <main className="p-8 space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-[0_2px_12px_0_rgba(0,0,0,0.05)] border border-gray-100 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-50 flex items-center justify-between">
            ...
            <Link href="#" className="text-sm text-gray-400 flex items-center gap-1 hover:text-[#137fec] transition-colors">
              查看全部 
            </Link>
          </div>
          ...
          <div className="px-6 py-4 border-t border-gray-50 flex items-center justify-between">
            <p className="text-xs text-gray-400">
              显示第 {(page - 1) * pageSize + 1} 至 {Math.min(page * pageSize, total)} 项，共 {total} 项
            </p>
            <div className="flex items-center gap-2">
              <Link 
              ...
              <button className="px-3 py-1 bg-[#137fec] text-white text-xs font-bold rounded-lg">{page}</button>
              {total > page * pageSize && (
                <Link 
