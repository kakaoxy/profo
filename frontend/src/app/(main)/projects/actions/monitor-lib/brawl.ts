...
export async function getCompetitorsBrawlAction(projectId: string) {
  try {
    ...
    // 目标小区列表 (包含本项目小区)
    const targetCommunities = [project.community_name, ...competitorCommunities].filter(Boolean);
    ...
    if (project.status !== 'sold' && project.status !== 'archived') {
       ...
       const listPrice = project.list_price ? Number(project.list_price) : (project.signing_price ? Number(project.signing_price) : 0);
