...
export function TrendPositioning({
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const res:
          | Awaited<ReturnType<typeof getTrendPositioningAction>>
          | Awaited<ReturnType<typeof getTrendPositioningByCommunityAction>> =
          projectId
  ...
  // 简单的风险偏离计算
  const latestDealPrice =
    data.length > 0 ? data[data.length - 1].deal_price : 0;
  const riskPercent =
    latestDealPrice > 0 && myPricing > 0
          ...
          <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6 pt-6 border-t border-slate-50">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-bold text-slate-700">
                  您的定价 {Number(riskPercent) > 0 ? "高于" : "低于"}
                  最新成交均价 {Math.abs(Number(riskPercent))}%
                </p>
              </div>
            </div>
          </div>
