  ...
  return (
    <div className="space-y-4">
      ...
      <InfoSection title="交易数据" icon={<TrendingUp className="h-4 w-4" />}>
          ...
          value={
            (project.netCashFlow ?? project.net_cash_flow) !== undefined ? (
              <span
                className={cn(
                  "font-bold font-mono",
                  (project.netCashFlow ?? project.net_cash_flow ?? 0) >= 0 ? "text-green-600" : "text-red-500"
                )}
              >
                {formatPrice(((project.netCashFlow ?? project.net_cash_flow) || 0) / 10000)}
              </span>
            ) : undefined
          }
        />
        ...
        <InfoItem label="签约日期" value={formatDate(project.signingDate || project.signing_date)} />
        <InfoItem
          label="计划交房"
          value={formatDate(project.plannedHandoverDate || project.planned_handover_date)}
        />
        <InfoItem 
          label="成交日期" 
