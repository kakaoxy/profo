...
import type { GrowthCampaign } from "../../_lib/campaign-data";

interface CampaignsTableProps {
  campaigns: GrowthCampaign[];
  onEdit: (campaign: GrowthCampaign) => void;
  onToggleStatus: (campaign: GrowthCampaign) => void;
  onQr: (campaign: GrowthCampaign) => void;
  onDelete: (campaign: GrowthCampaign) => Promise<ActionResult<void>>;
...
/**
 * 活动配置表格（对齐设计稿 Screen 4 列结构）：
 * `# / 活动名称+ID / 分享标题 / 分享配图（5:4）/ 状态 / 创建时间 / 操作`。
 * 行操作：编辑 / 小程序码 / 停用（停用中显示「启用」）/ 删除，写操作按权限隐藏。
 */
