...
import type { GrowthCampaign, GrowthCampaignStatus } from "../../_lib/campaign-data";
import { uploadCampaignImageAction } from "../../_lib/campaign-actions";
...
export function CampaignFormDialog({
  ...
  const [status, setStatus] = React.useState<GrowthCampaignStatus>("enabled");
  ...
  const handleSubmit = () => {
    // image_url 为 null 时归一为空串：z.string().min(1) 对 null 产生英文 invalid_type
    // 错误（自定义文案不生效），空串则命中 min(1) 的中文提示
