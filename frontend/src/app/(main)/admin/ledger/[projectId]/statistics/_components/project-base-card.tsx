...
export function ProjectBaseCard({ data }: ProjectBaseCardProps) {
  const dealDate = data.deal_date ? formatDate(data.deal_date) : "未成交";
  ...
  return (
    <div className="bg-white rounded-cards p-6 shadow-[rgba(4,23,43,0.04)_0px_0px_0px_1px,rgba(0,0,0,0.06)_0px_12px_16px_-4px] h-full animate-in" style={{ animationDelay: "0.25s" }}>
      {/* Section header */}
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <p className="text-[14px] leading-normal mb-1 text-graphite">
              小区名称
            </p>
            <p className="text-[22px] leading-tight text-ink truncate">
              {data.community_name || "-"}
            </p>
          </div>
          {data.status ? (
        ...
        {/* 交房时间 */}
        <DataRow label="交房时间" value={formatDate(data.delivery_date)} />

        {/* 成交时间 */}
        <DataRow label="成交时间" value={dealDate} />

        {/* 项目天数高亮块 */}
        <div className="mt-6 p-5 rounded-inputs bg-apricot-wash">
...
function DataRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between items-center py-2.5 border-b border-dove/25 last:border-b-0">
      ...
      <span className="text-[15px] font-medium text-ink text-right wrap-break-word">
        {value}
      </span>
    </div>
  );
