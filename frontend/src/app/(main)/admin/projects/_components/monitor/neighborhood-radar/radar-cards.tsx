  ...
  return (
    <div className="sm:hidden divide-y divide-border">
      {competitors.map((item) => (
        <div key={item.community_id} className={`p-4 ${item.is_subject ? "bg-primary/5" : ""}`}>
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            <div>
              <p className="text-graphite mb-0.5">挂牌</p>
              <p className="font-bold text-ink">{item.listing_count} 套</p>
              <p className="text-primary font-bold">
                ¥{item.listing_avg_price.toLocaleString()}/㎡
              </p>
            </div>
            <div>
              <p className="text-graphite mb-0.5">成交</p>
              <p className="font-bold text-ink">{item.deal_count} 套</p>
              <p className="text-success font-bold">¥{item.deal_avg_price.toLocaleString()}/㎡</p>
            </div>
          </div>
        </div>
      ))}
