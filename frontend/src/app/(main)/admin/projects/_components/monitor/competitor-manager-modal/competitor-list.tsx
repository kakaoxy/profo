...
export function CompetitorList({
  ...
  if (competitors.length === 0) {
    return (
      <div className="text-center py-8 text-sm text-graphite bg-fog rounded-lg">
        暂未添加竞品小区
      </div>
    );
  }
  ...
  return (
    <div className="border border-fog rounded-lg divide-y divide-border">
      {competitors.map((item) => (
        <div key={item.community_id} className="px-4 py-3 flex items-center justify-between">
          <div>
            <div className="font-medium text-sm text-ink">{item.community_name}</div>
