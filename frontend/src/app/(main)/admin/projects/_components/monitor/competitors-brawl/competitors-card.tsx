...
export function CompetitorsCard({ items }: CompetitorsCardProps) {
  return (
    <div className="divide-y divide-border">
      {items.map((item) => (
        <div key={item.id} className={`p-4 ${item.is_current ? "bg-primary/5" : ""}`}>
          <div className="flex justify-between items-start mb-2">
            <div>
              <span className="text-sm font-bold text-ink">{item.community}</span>
              <span
            ...
            <span className="text-[10px] text-graphite font-mono">{item.date}</span>
          </div>
          ...
          <div className="flex justify-between items-center">
            <div>
              <span className="text-lg font-black text-destructive">¥{item.total}万</span>
              <span className="ml-2 text-xs text-graphite">¥{item.unit.toLocaleString()}/㎡</span>
            </div>
