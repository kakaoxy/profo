...
export function UserTable({
  ...
  const renderSortableHead = (label: string, field: string) => (
    <TableHead className="cursor-pointer select-none hover:text-ink" onClick={() => onSort(field)}>
  ...
  return (
    <>
      {/* 表格卡壳由父级 users-client 的 Steep 卡片提供；ui/table 自带横向滚动容器 */}
      <Table>
        <TableHeader>
          <TableRow>
          ...
          {data.map((user) => {
            ...
            return (
              <TableRow key={user.id}>
                <TableCell>
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-graphite">{user.username}</span>
                    </div>
                  </div>
                </TableCell>
                <TableCell>
                  <div className="flex flex-wrap items-center gap-1">
                    ...
                    {user.additional_roles?.map((role) => (
                      <span
                        key={role.id}
                        className="ml-1 rounded border border-dashed border-fog px-1.5 py-0.5 text-xs text-graphite"
                      >
                        +{role.name}
                      </span>
                    ))}
                ...
                <TableCell>
                  {user.leads_count > 0 ? (
                    <Link href={`/admin/leads?creator_id=${user.id}`} className="users-leads-link">
                      {user.leads_count}
                    </Link>
                  ) : (
                ...
                <TableCell className="text-sm text-graphite">{user.phone || "-"}</TableCell>
