...
/**
 * 旧路径兼容层：/admin/recruit/funnel → /admin/growth-center/funnel。
 * 获客中心信息架构迁移后的服务端重定向，透传查询参数。
 */
export default async function RecruitFunnelRedirectPage({
  searchParams,
}: {
  searchParams: Promise<SearchParams>;
}) {
  const qs = toQueryString(await searchParams);
  redirect(`/admin/growth-center/funnel${qs ? `?${qs}` : ""}`);
}
