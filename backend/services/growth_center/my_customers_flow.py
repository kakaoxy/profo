...
- valuation/sheet：仅支持 eliminated 淘汰旁路（reason 必填），按原因映射回写
  LeadStatus 并写 audit_time/auditor_id（行级锁对齐 ``authorize_assessment`` 模式）；
- booking：预约状态机二期，一律 409。
...
from models import Lead, RecruitLead, RecruitLeadStatus, User
...
# 统一状态流转矩阵（目标状态集合；终态为空集 → 含流转到自身在内一律拒绝）
_TRANSITIONS: dict[UnifiedLeadStatus, set[UnifiedLeadStatus]] = {
    ...
    UnifiedLeadStatus.ELIMINATED: set(),
}
...
class MyCustomerFlowService:
    ...
    def update_status(
        ...
        *,
        ...
        Raises:
            ConflictError: booking 一律 409 / 流转矩阵不合法 / 估价线非淘汰流转
            ResourceNotFoundError: 线索不存在或不归属当前用户
            BusinessLogicError: 淘汰旁路缺少 reason（422）

        """
        if module == GrowthModule.BOOKING:
            msg = "预约状态机二期，暂不支持手动流转"
            raise ConflictError(msg)
    ...
    def _update_lead_status(
        ...
        *,
        ...
        """估价/房源单线流转：仅支持 eliminated 淘汰旁路（行级锁防并发）.
        ...
        self.db.commit()
        self.db.refresh(lead)
        return {"unified_status": UnifiedLeadStatus.ELIMINATED, "native_status": lead.status.value}
