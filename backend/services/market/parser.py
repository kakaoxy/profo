...
_FLOOR_LOW_RATIO = 0.33
_FLOOR_MID_RATIO = 0.67
...
class FloorParser:
    ...
    def parse_floor(floor_original: str) -> FloorInfo:  # noqa: C901
        ...
        # 尝试提取当前楼层数，匹配模式: "15/28", "15层/28层", "第15层"
        floor_patterns = [
            r"^(\d+)\s*/\s*\d+",
            r"^(\d+)\s*层",
            r"第\s*(\d+)\s*层",
        ...
        if (
            floor_info.floor_number is not None
            and floor_info.total_floors is not None
            and floor_info.floor_level is None
        ):
    ...
    def calculate_floor_level(floor_number: int, total_floors: int) -> str:
        ...
        if ratio <= _FLOOR_LOW_RATIO:
            return FloorParser.LEVEL_LOW
        if ratio <= _FLOOR_MID_RATIO:
            return FloorParser.LEVEL_MID
