...
class L3ProjectBriefResponse(BaseModel):
    ...
    area: Optional[Decimal] = Field(None, description="面积(m²)")
...
class L3ProjectImportResponse(BaseModel):
    ...
    # 面积与价格
    area: Optional[Decimal] = Field(None, description="面积(m²)")
    total_price: Optional[Decimal] = Field(None, description="总价(万元)")
    unit_price: Optional[Decimal] = Field(None, description="单价(万元/m²)")
...
class L3ProjectQueryParams(BaseModel):
    """L3项目查询参数"""
    community_name: Optional[str] = Field(None, description="小区名称筛选")
