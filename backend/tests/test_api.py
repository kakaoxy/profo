...
from models import Base, Community, PropertyCurrent, PropertyStatus, User, Role
from db import get_db
from utils.auth import get_password_hash


# 测试数据库配置 - 使用文件数据库而非内存数据库
...
@pytest.fixture
def normal_user(client, sample_roles):
...
@pytest.fixture
def sample_data(client, sample_roles, normal_user):
    ...
    try:
        ...
        db.add_all([community1, community2])
        db.commit()
        db.refresh(community1)
        db.refresh(community2)
...
def test_query_all_properties(client, sample_data, auth_headers):
...
def test_filter_by_status_for_sale(client, sample_data, auth_headers):
    """Test filtering by status (在售) - Requirement 4.1"""
    response = client.get("/api/properties", params={"status": "在售"}, headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 3
...
def test_filter_by_status_sold(client, sample_data, auth_headers):
    """Test filtering by status (成交) - Requirement 4.1"""
    response = client.get("/api/properties", params={"status": "成交"}, headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 2
    for item in data["items"]:
...
def test_filter_by_community_name(client, sample_data, auth_headers):
    """Test filtering by community name - Requirement 4.2"""
    response = client.get("/api/properties", params={"community_name": "测试小区A"}, headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 3


def test_filter_by_price_range(client, sample_data, auth_headers):
...
def test_filter_by_area_range(client, sample_data, auth_headers):
    """Test filtering by area range - Requirement 4.4"""
    response = client.get("/api/properties", params={"min_area": 80, "max_area": 120}, headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
    # Properties: prop2 (85.0), prop5 (110.0) - prop1 (120.5) is out of range
    assert data["total"] == 2


def test_filter_by_rooms(client, sample_data, auth_headers):
    """Test filtering by room types - Requirement 4.5"""
    response = client.get("/api/properties", params={"rooms": "2,3"}, headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 4


def test_sort_by_price_ascending(client, sample_data, auth_headers):
...
def test_pagination(client, sample_data, auth_headers):
    """Test pagination - Requirement 4.8"""
    response = client.get("/api/properties", params={"page": 1, "page_size": 2}, headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 5
    assert data["page"] == 1
...
def test_calculated_unit_price(client, sample_data, auth_headers):
...
def test_calculated_transaction_duration(client, sample_data, auth_headers):
...
def test_unauthenticated_request(client, sample_data):
