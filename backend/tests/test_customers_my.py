...
招募/估价/预约状态流转矩阵、估价淘汰原因回写映射（audit 轨迹）、
...
from models import Lead, LeadStatus, RecruitLead, RecruitLeadStatus, User
...
@pytest.mark.parametrize("target", ["contacted", "high_intent", "converted", "eliminated"])
def test_booking_status_always_409(
    target: str,
    c_client_factory: Callable[[User], TestClient],
    my_customers_data: dict[str, Any],
    c_users: dict[str, User],
) -> None:
    """预约状态机二期：任意目标状态流转一律 409."""
    client = c_client_factory(c_users["a"])
    resp = client.put(f"{BASE}/booking/{my_customers_data['booking_id']}/status", json={"status": target})
    assert resp.status_code == 409
