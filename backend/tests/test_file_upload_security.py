...
class TestSafeFilePath:
    ...
    def test_normal_filename_returns_safe_path(self, tmp_path: Path) -> None:
    ...
    def test_sanitize_prevents_traversal_then_is_safe(self, tmp_path: Path) -> None:
        from utils.file_security import get_safe_file_path
    ...
    def test_traversal_with_extension_uses_sanitized_name(self, tmp_path: Path) -> None:
        from utils.file_security import get_safe_file_path
    ...
    def test_windows_traversal_sanitized(self, tmp_path: Path) -> None:
        from utils.file_security import get_safe_file_path
    ...
    def test_is_safe_path_rejects_absolute_outside_path(self, tmp_path: Path) -> None:
...
class TestSanitizeFilenameReturnUsed:
    ...
    def test_sanitize_filename_return_value_assigned(self) -> None:
        import inspect
        ...
        assert has_assignment, (
            "upload_file() 中 sanitize_filename() 返回值未被接收。必须将返回值赋给变量或直接用于 os.path.splitext()"
        )

    def test_extension_from_sanitized_not_raw_filename(self) -> None:
        import inspect
        ...
        assert "get_safe_file_path(" in source, "upload_file() 必须调用 get_safe_file_path() 验证最终保存路径"

    def test_double_extension_handled_correctly(self) -> None:
    ...
    def test_path_traversal_sanitized_before_extension_extraction(self) -> None:
        from utils.file_security import sanitize_filename
...
class TestUploadFileIntegration:
    ...
    def test_get_safe_file_path_called_during_upload(self) -> None:
        ...
        with (
            patch.object(files_module, "get_safe_file_path", return_value=fake_safe_path) as mock_get_safe,
