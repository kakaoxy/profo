...
- [1. 小区管理](#1-小区管理)
  ...
  - [1.3 查询商圈列表（按区域过滤）](#13-查询商圈列表按区域过滤)
  - [1.4 合并小区](#14-合并小区)
  - [1.5 创建小区](#15-创建小区)
- [2. 房源查询](#2-房源查询)
  - [2.1 搜索小区](#21-搜索小区)
...
返回去重商圈字典列表，支持按区域（行政区）精确过滤与名称模糊搜索。
用于筛选器的「区域 → 商圈」两级联动（区域切换后按区域重新拉取商圈）。

```
GET /api/v1/admin/business-circles
```

**认证要求**：property:read 权限

**Query 参数**

| 参数 | 类型 | 必填 | 约束 | 默认值 | 说明 |
|------|------|------|------|--------|------|
| district | string | 否 | ≤ 100 字符 | - | 区域（行政区）精确过滤 |
| search | string | 否 | ≤ 100 字符 | - | 商圈名称模糊搜索 |
| limit | integer | 否 | 1–200 | 200 | 返回数量上限 |

**请求示例**

```http
GET /api/v1/admin/business-circles?district=徐汇区&limit=200
Authorization: Bearer <token>
```
